...
void sys_initDisk() {
    _virtualDiskAddr = (char *)malloc(_virtualDiskSize * sizeof(char));   //声请空间：char是一个字节，所以虚拟磁盘的大小是8M；
...
/* //初始化盘块的位式图:'0'表示未被使用，'1’表示已经使用
	...
	*/
    ...
    _lastFCB = _emptyFCB;
    for (int i = 1; i < _FCB_count - 1; i++)   //还剩_FCB_count-1个FCB，每个64B；
    {
...
/////////////////////////////////////////////////
BLOCK* num2Block(int num) {
    BLOCK* B = (BLOCK*)(_virtualDiskAddr + num*_blockSize);
    return B;
}
/////////////////////////////////////////////
...
int getBLOCK(int need_BLOCK_Num) {
    if (_BLOCK_count >= need_BLOCK_Num)
    {
        ////block记住分配的头
        int block;
...
*/
//删除文件时，回收并清理BLOCK；
void recoverBLOCK(int block) {
    if (block == -1)
        return;
    int next_block;
    while (_fat->fats[block] != -1) {
        next_block = _fat->fats[block];
        _fat->fats[block] = -1;
        BLOCK* rmBlock = num2Block(block);
        strcpy(rmBlock->_content, "");
        rmBlock->_blockLength = 0;
        _emptyBLOCK_Count++;
        block = next_block;
    }
}
...
//删除目录和文件中有个相同的操作就是回收FCB，现抽取出来；////修改了代码位置
...
void add_dirOrFile(FCB* adding, FCB* destination) {
    ...
    if (tmp == NULL)
    {
        ...
        else
        {
            ...
            if (needAddBLOCK == tmp)
            {
                ...
                contentFirstBlockNum = getBLOCK(tmp);
            ...
            strcpy(contentFirstAdrr->_content, ((string)(tmp_addingBLOCK->_content)).substr(0, contentFirstAdrr->_blockLength).c_str());
            /*
            //先处理整数内容的BLOCK；
            for (int i = 0; i < tmp - 1; i++)
            {//////？？？？？？？？？？？？？？？？？
    ...
    else
    {
        ...
        while (tmp != NULL)
        {
            //本质是拷贝文件，所以文件/目录实现方式相同；递归调用--注意递归出口也即统一性
            add_dirOrFile(tmp, copy_adding);
            tmp = tmp->_brother;
        }
    }
...
//查看当前目录内容；//stringlist返回打印内容(正确内容or目录不存在)
...
StringList* sys_dir(string dirPath) {
    ...
    StringList *dir_content = new StringList();//用StringList 存取需要打印的信息；
    ...
    else tmp = _current;//？？？？当前怎么确定的


    if (tmp != NULL) {//要查看的目录存在；

        tmp_dirContent = dir_content;//tmp_dirContent，dir_content的临时副本，由于是指针内容写入的是同一地址
        tmp = tmp->_child;//tmp指针变了当前目录
...
int sys_mkdir(string dirPath) {
    ...
    if (sys_returnFCB(dirPath, dirName, dir_type, parentDir) == NULL) {
        ...
        if (parentDir != NULL)
        {
            ...
            strcpy(dirByUser->_name, dirName.c_str());
...
int sys_create_file(string filePath) {
    ...
    if (sys_returnFCB(filePath, fileName, file_type, parentDir) == NULL) {
        ...
        if (parentDir != NULL)
        {
            ...
            strcpy(fileByUser->_name, fileName.c_str());
...
int sys_write_file(FCB* file, string content) {
    ...
    {
        //说明没损失，所以分配的个数为tmp;
        contentFirstBlockNum = getBLOCK(needUsedBlock);
...
int main() {
    sys_initDisk();
    //if (sys_mkdir("/m") == 1)cout << "success";
    string m;
    FCB* p;
    //if(sys_cd("/m",t)==1) cout<<"cd";
    //sys_create_file("/m/t");
    FCB *fcb1 = getFCB();
    FCB *fcb2 = getFCB();
    strcpy(fcb1->_name, "t1");
    strcpy(fcb2->_name, "t2");
    //_root->_child=fcb;
    if (sys_write_file(fcb1,"hello") == 1) cout << "ok1"<<endl;

    cout<<fcb1->_name<<"!!"<<endl;
    cout<<fcb2->_name<<"??"<<endl;
    cout<<sys_read_file(fcb1)<<endl;
    if (sys_write_file(fcb2,"1234") == 1) cout << "ok2"<<endl;
    cout<<sys_read_file(fcb2)<<endl;
    cout<<sys_read_file(fcb1)<<endl;
    BLOCK* b1=num2Block(128);
    cout<<b1->_content<<"111"<<endl;
    BLOCK* b2=num2Block(129);
    cout<<b2->_content<<"222"<<endl;

    //sys_copy("/t",0,"/m");
    //FCB* q=sys_returnFCB("/m",m,0,p);
    //cout<<sys_read_file(q);
    //cout<<sys_dir("/");
}