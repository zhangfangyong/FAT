...
#define _FCB_size 64                //FCB定长管理，64B；//则一个磁盘块可以存储16个；
//#define _bitMapCount 8                  //位式图占用的磁盘块个数
//分配设定x个磁盘块（同时设定了最多文件数）存储目录树；
//0-7磁盘块位式图；8-127；(认为0-127，是系统空间)剩余的磁盘块都用来存储文件内容，通过指针指向；128-8191(用户空间)；
#define _systemUsedBlock 128               //系统使用的block总数；
#define _FatBlockCount 32                  //位式图占用的磁盘块个数
#define _FCB_count 1536                  //FCB总个数；（128-32）*16
#define _BLOCK_count 8064                  //BLOCK总个数；

/*
struct fat {
	BLOCK* _Block;
	//盘块数与地址的换算 首+num*_blockSize
	//指针存地址？
	int next;
	fat() {
		this->next = -1;
...
struct FAT {
    //fat fats[8*1024];//总盘块数
    int fats[_blockCount];//32盘块//最好赋值全-1？？？？？
    FAT() {
